...
from easy_tdx.exceptions import TdxConnectionError, TdxDecodeError
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    @app.exception_handler(TdxDecodeError)
    async def tdx_decode_error_handler(
        request: Request, exc: TdxDecodeError
    ) -> JSONResponse:
        del request
        return JSONResponse(
            status_code=502,
            content=ApiErrorResponse(
                error="TDX decode error",
                detail=str(exc),
            ).model_dump(),
        )
