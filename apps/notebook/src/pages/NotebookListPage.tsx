...
import { sourceLabelKey } from '@/lib/data-sources'
            ...
            <Card key={nb.id} className="group cursor-pointer transition-colors hover:border-primary/50" onClick={() => navigate({ to: '/nb/$id', params: { id: nb.id } })}>
              ...
              <CardContent>
                <div className="mb-3 text-xs text-muted-foreground">
                  {t(sourceLabelKey(nb.dataSource ?? 'api'))} · {nb.symbol} · {nb.timeframe} · {nb.cells.length} {t('list.cells')} · {nb.kernelLang.toUpperCase()}
                </div>
