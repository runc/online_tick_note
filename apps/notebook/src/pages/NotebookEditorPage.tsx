...
import {
  Play, Plus, Save, Download, Trash2, Code2, FileText, BarChart3,
} from 'lucide-react'
...
import { DataSourceBar } from '@/components/editor/DataSourceBar'
import { useNotebookStore, useCellStore, normalizeNotebook } from '@/stores/notebook-store'
...
import { getDefaultDataSource, getDefaultSymbol, getEnabledDataSources, getTimeframesForSource } from '@/lib/data-sources'
...
export function NotebookEditorPage() {
  ...
  const dataSource = currentNotebook?.dataSource ?? 'api'
  const symbol = currentNotebook?.symbol ?? ''
  const timeframe = currentNotebook?.timeframe ?? '1d'
  const { data: dataset, isLoading: dataLoading, error: datasetError } = useDataset(symbol, timeframe, dataSource)

  useEffect(() => {
    loadNotebook(id).then((nb) => {
      if (nb) setCurrentNotebook(normalizeNotebook(nb))
      else navigate({ to: '/' })
    })
  ...
  useEffect(() => {
    ...
    if (enabled.includes(currentNotebook.dataSource)) return
    const next = getDefaultDataSource()
    updateNotebook({
      dataSource: next,
      symbol: getDefaultSymbol(next),
      timeframe: getTimeframesForSource(next)[0] ?? '1d',
    })
  }, [currentNotebook, easyTdxEnabled, builtinSamplesEnabled, updateNotebook])
  ...
  const handleDataSourceChange = (source: DataSourceKind) => {
    updateNotebook({ dataSource: source })
  }

  const handleSymbolChange = (sym: string, tf: string) => {
    updateNotebook({ symbol: sym, timeframe: tf })
  }
  ...
  const handleCsvUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        updateNotebook({ dataSource: 'csv', symbol: sym, timeframe: '1d' })
  ...
  const dataErrorMessage = datasetError instanceof Error ? datasetError.message : datasetError ? String(datasetError) : null
  ...
  return (
    <div className="flex h-full flex-col">
      <header className={cn(APP_HEADER_BAR, 'min-w-0 gap-2')}>
        <Input
          value={currentNotebook.title}
          onChange={(e) => updateNotebook({ title: e.target.value })}
          className="h-8 w-32 shrink-0 border-none bg-transparent text-sm font-medium focus-visible:ring-0"
        />
        ...
        <div className="min-w-0 flex-1 basis-[min(100%,28rem)]">
          ...
          barCount={dataset?.meta.barCount}
