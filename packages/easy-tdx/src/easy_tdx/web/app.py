...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    host_explicit = getattr(app.state, "tdx_host_explicit", False)

    try:
        if host_explicit:
            client = AsyncTdxClient(host=host, port=port, timeout=timeout)
        else:
            client = AsyncTdxClient.from_best_host(port=port, timeout=timeout)
        await client.connect()
        logger.info("TDX client connected to %s:%s", client._host, port)
    except Exception:
...
def _create_app(
    ...
    *,
    ...
    app.state.tdx_host_explicit = explicit_host
